    ...
    public class DailyCPUDataRepository
    {
        ...
        public void CreateDailyCPUDatas()
        {
            try
            {
                ...
                NHibernateHelper.CreateTables(_connectionString, typeof(DailyCPUDataMap).Assembly);
            }
            catch (Exception ex) {
                if (ex.InnerException != null && ex.InnerException.Message.Contains("already exist"))
                    throw ex;
            }
        ...
        public void CreateDailiesTopProcessesTable()
        {
            try
            {
                NHibernateHelper.CreateTables(_connectionString, typeof(DailiesTopProcessMap).Assembly);
            }
            catch (Exception ex)
            {
                if (ex.InnerException != null && ex.InnerException.Message.Contains("already exist"))
